...
async function main() {
  ...
  const token = await StableToken.deploy("BuildBear USD", "USD");
  await token.deployed();
  console.log(`USD deployed at ${token.address}`);

  const tx = await token.mint(recipient, mintAmount);
  const receipt = await tx.wait();
  console.log(`Minted ${amountLabel} USD to ${recipient} in tx ${receipt.transactionHash}`);

  const runRecord = {
    timestamp: new Date().toISOString(),
    network: hre.network.name,
    script: "scripts/deploy-and-mint-usd.js",
    contractName: "BuildBear USD",
    contractSymbol: "USD",
  ...
  const dbFile = path.join(dbDir, "usd-deploy-mint-runs.json");
