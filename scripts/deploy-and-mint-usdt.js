...
async function main() {
  ...
  const token = await StableToken.deploy("Tether USD", "USDT");
  await token.deployed();
  console.log(`USDT deployed at ${token.address}`);

  const tx = await token.mint(recipient, mintAmount);
  const receipt = await tx.wait();
  console.log(`Minted ${amountLabel} USDT to ${recipient} in tx ${receipt.transactionHash}`);

  const runRecord = {
    timestamp: new Date().toISOString(),
    network: hre.network.name,
    script: "scripts/deploy-and-mint-usdt.js",
  ...
  const dbFile = path.join(dbDir, "usdt-deploy-mint-runs.json");
