...
async function main() {
  ...
  const token = await StableToken.deploy("Wrapped Bitcoin", "WBTC");
  await token.deployed();
  console.log(`WBTC deployed at ${token.address}`);

  const tx = await token.mint(recipient, mintAmount);
  const receipt = await tx.wait();
  console.log(`Minted ${amountLabel} WBTC to ${recipient} in tx ${receipt.transactionHash}`);

  const runRecord = {
    timestamp: new Date().toISOString(),
    network: hre.network.name,
    script: "scripts/deploy-and-mint-wbtc.js",
  ...
  const dbFile = path.join(dbDir, "wbtc-deploy-mint-runs.json");
